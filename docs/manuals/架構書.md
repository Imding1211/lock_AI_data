...
本文件概述了為「電子鎖 AI 客服系統」量身打造的 RAG 數據中台架構。系統採用 Medallion Architecture (獎章架構)，旨在將多源、異質且含有雜訊的原始資料（如 Line 聊天、語音轉錄、網頁、YouTube），轉化為高品質、結構化且具備高度語義檢索能力的 pgvector 向量知識庫。
...
### 3.3 Silver 層作為絕對中樞 (The Silver Hub)
Silver 層是整個架構的分水嶺：
...
*   **Silver 之後**：通用泛化。`seed_pgvector.py` 完全不關心資料來源，它遞迴讀取所有 Silver JSON，執行切塊 (Chunking) 並寫入 Gold 層，達成最高程度的程式碼復用。

## 4. 數據生命週期詳解

### 4.1 Bronze to Silver (客製化清洗中台)
以 `process_video.py` 為例：
1.  **冪等性檢查**：檢查目標 JSON 是否已存在，避免重複消耗 LLM 運算資源。
2.  **LLM 介入**：將原始 ASR 文本送入 LLM Factory，要求其扮演「資深電子鎖技術編輯」，執行語音辨識糾錯（如「掌機賣」修正為「掌靜脈」）與冗言去噪。
3.  **結構化輸出**：透過 API 強制 LLM 輸出純 JSON 格式。
...
2.  將 `storage/silver/` 下的 JSON 轉為 `Document` 物件。
3.  使用 `RecursiveCharacterTextSplitter` 進行語義切塊 (Chunking)。
4.  寫入 pgvector，供 AI Agent 透過 `brand` 等 metadata 進行精確過濾與檢索。
